...
```{r}
BODATSA <- BODATSA %>% unite("canonicalName", c(Genus, Sp1, Rank1, Sp2, Rank2, Sp3), na.rm = TRUE, sep = " ", remove = FALSE)
```
...
### Remove misapplied taxon names (they are not linked to accepted names)

```{r}
BODATSA<-BODATSA %>% filter(TaxStat != "misap") # filter out misapplied names
```

### Link unused autonyms to their accepted names, and flag if also a synonym
...
### Identify the lists of taxon names that need to be standardised

```{r}
Inputs<-list.files("data//interim//", pattern = "List_s.csv") # files that need to be standardised
```

# Steps 3-14: Iteratively taxonomically standardise lists

The taxon names in the lists are taxonomically standardised iteratively, with all subsequent steps of workflow run within a loop

```{r}
### Load lists of names for standardisation and create required sub folders
for(q in 1:length(Inputs)){
  FName<-Inputs[q]
  NamesDat<-read.csv(paste0("data//interim//", FName, sep = "")) # read in list of taxon names
  DName<-unlist(str_split(Inputs[q], "_"))[1]
  dir.create(file.path("data/interim", DName)) # create subfolder in interim
  dir.create(file.path("data/processed", DName)) # create subfolder in outputs
...
write.csv(GBIFInterim, paste0("data//interim//", DName, "//GBIFInterim.csv"), row.names = FALSE)
...
## Create a subset of taxa that are not vascular plants
...
GBIFMatch <- GBIFMatch %>%  mutate(openNomenclature.flag=case_when((str_detect(verbatimScientificName,fixed("sp. "))) ~ TRUE,
                                    ...
                                    TRUE~FALSE)) # TRUE if a open nomenclature
...
### Replace synonyms and unused autonyms in "scientificName" column with accepted names from "Accepted" column
...
write.csv(BODATSAInterim, paste0("data//interim//", DName, "//BODATSAInterim.csv"), row.names = FALSE)
...
#### Flag unmatched taxa (those unmatched, or not matched at the same level, includes unused autonyms)
...
BODATSAMatch <- BODATSAMatch %>%  mutate(openNomenclature.flag=case_when((str_detect(verbatimScientificName, fixed("sp. "))) ~ TRUE,
                                                                         (str_detect(verbatimScientificName, fixed("spp. "))) ~ TRUE,
                                                                         (str_detect(verbatimScientificName, fixed("cf. "))) ~ TRUE,
                                                                         (str_detect(verbatimScientificName, fixed("nr. "))) ~ TRUE,
                                                                         (str_detect(verbatimScientificName, fixed("aff. "))) ~ TRUE,
...
write.csv(WCVPInterim, paste0("data//interim//", DName, "//WCVPInterim.csv"), row.names = FALSE)
...
### Flag doubtful taxon names (e.g., invalid, illegitimate, missapplied, and those with unresolved multiple matches, or only matched at genus level)
...
WCVPMatch <- WCVPMatch %>%  mutate(openNomenclature.flag=case_when((str_detect(verbatimScientificName,fixed("sp. "))) ~ TRUE,
                                                                   (str_detect(verbatimScientificName, fixed("spp. "))) ~ TRUE,
                                                                   (str_detect(verbatimScientificName, fixed("cf. "))) ~ TRUE, 
                                                                   (str_detect(verbatimScientificName, fixed("nr. "))) ~ TRUE, 
                                                                   (str_detect(verbatimScientificName, fixed("aff. "))) ~ TRUE, 
...
write.csv(AllMatch, paste0("data//processed//", DName, "//Taxon_names_standardised.csv"), row.names = FALSE)
}
```
